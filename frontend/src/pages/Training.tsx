    ...
    <div className="register-container">
      <div className="glass-panel" style={{ maxWidth: '800px', width: '100%' }}>
        <div className="page-head">
          <div>
            <div className="page-eyebrow">Newtype Lab</div>
            <h1 className="page-title">フラナガン機関</h1>
          </div>
          <button onClick={() => navigate('/mypage')} className="btn sm">マイページへ</button>
        </div>

        <div className="npc-dialog">
          <img className="npc-portrait" src="/images/npc/dr_flanagan.png" alt="フラナガン博士" />
          <div className="npc-body">
            <div className="npc-name">フラナガン博士 ─ ニュータイプ研究所</div>
            {canAwaken ? (
              user.nt_level === 0 ? (
                <>
                  <div className="npc-quote">「君はさらに強くなりたいのだね。ただし、それには条件がある。この条件でよければ、協力しよう。」</div>
                  <div className="npc-note">※ 必要な資金: {nyu_kin}G</div>
                </>
              ) : (
                <>
                  <div className="npc-quote">「さらに能力の極致へ向かいたいのだな。」</div>
                  <div className="npc-note">※ 必要な資金: {nyu_kin}G</div>
                </>
              )
            ) : (
              <>
                <div className="npc-quote">「ここの施設を真に利用するには、熟練度（Lv.10以上）か、全ステータス（20以上）が足りないようだな。」</div>
                <div className="npc-note">（現在のLv: {user.level}, 所持金: {user.money}G）</div>
              </>
            )}
          </div>
        </div>
        ...
        <div className="inset-panel" style={{ marginBottom: '1.5rem' }}>
          <h3 className="sec-title">現在の覚醒状態</h3>
          <div className="row-wrap" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>タイプ</div>
              <div style={{ fontSize: '1.1rem', color: user.nt_level > 0 ? 'var(--accent-cyan)' : user.nt_level < 0 ? 'var(--danger)' : 'var(--text-primary)', fontWeight: 'bold' }}>
                {user.nt_level > 0 ? 'ニュータイプ (NT)' : user.nt_level < 0 ? '強化人間' : 'オールドタイプ (未覚醒)'}
              </div>
            </div>
            <div>
              <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>覚醒レベル</div>
              <div style={{ fontSize: '1.1rem', fontWeight: 'bold' }}>
                {Math.abs(user.nt_level)}
              </div>
            </div>
            <div>
              <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>所持金</div>
              <div style={{ fontSize: '1.1rem', color: 'var(--warning)', fontWeight: 'bold' }}>
                {user.money.toLocaleString()} G
              </div>
            </div>
            <div>
              <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>名声</div>
              <div style={{ fontSize: '1.1rem', color: 'var(--warning)', fontWeight: 'bold' }}>
                {user.fame || 0}
              </div>
            </div>
        ...
        {canAwaken && (
          <div className="grid-2" style={{ marginBottom: '1.5rem', gap: '1rem' }}>
            {user.nt_level >= 0 && user.nt_level < 5 && (
              <div className="inset-panel" style={{ borderColor: 'rgba(79, 172, 254, 0.4)', textAlign: 'center' }}>
                <h3 style={{ color: 'var(--accent-cyan)', marginBottom: '1rem', fontSize: '1rem' }}>ニュータイプ覚醒</h3>
                <p style={{ fontSize: '0.8rem', marginBottom: '1rem', minHeight: '40px' }}>
            ...
            {user.nt_level <= 0 && user.nt_level > -7 && (
              <div className="inset-panel" style={{ borderColor: 'rgba(239, 68, 68, 0.4)', textAlign: 'center' }}>
                <h3 style={{ color: 'var(--danger)', marginBottom: '1rem', fontSize: '1rem' }}>強化人間手術</h3>
                <p style={{ fontSize: '0.8rem', marginBottom: '1rem', minHeight: '40px' }}>
        ...
        <div className="inset-panel" style={{ marginBottom: '1.5rem' }}>
          <h3 className="sec-title">訓練・強化</h3>
          ...
          <div className="action-row">
            <div style={{ flex: '1', minWidth: '120px' }}>
              ...
              <select className="input-field" style={{ width: '100%' }} value={trainStat} onChange={(e) => setTrainStat(e.target.value)}>
            ...
            <div style={{ flex: '1', minWidth: '120px' }}>
              <label className="field-label">コース</label>
              <select className="input-field" style={{ width: '100%' }} value={trainCourse} onChange={(e) => setTrainCourse(e.target.value as 'normal'|'enhanced')}>
            ...
            <div style={{ flex: '1', minWidth: '80px' }}>
              ...
              <select className="input-field" style={{ width: '100%' }} value={trainKaisu} onChange={(e) => setTrainKaisu(Number(e.target.value))}>
                {[...Array(10)].map((_, i) => <option key={i+1} value={i+1}>{i+1}回</option>)}
              </select>
            </div>
            <div style={{ flex: '1', minWidth: '150px', alignSelf: 'flex-end' }}>
              <button onClick={handleTraining} disabled={user.money < trainCostPer * trainKaisu || (trainCourse === 'enhanced' && trainStatVal >= 170)} className="btn primary block">訓練・強化</button>
            </div>
          </div>
        </div>
        ...
        {user.awakening_suppressed === 0 && (
          <div className="inset-panel" style={{ marginBottom: '1.5rem' }}>
            <h3 className="sec-title">覚醒抑止</h3>
            <p style={{ fontSize: '0.9rem', marginBottom: '1rem' }}>
            ...
            <button onClick={handleSuppress} disabled={user.money < (5000 + Math.abs(user.nt_level) * 1000)} className="btn danger block">
              覚醒を抑止する
            </button>
          </div>
        )}
        ...
        {user.awakening_suppressed === 1 && (
          <div className="inset-panel" style={{ marginBottom: '1.5rem' }}>
            <h3 className="sec-title">オールドタイプ専用 技の習得</h3>
            <p style={{ fontSize: '0.9rem', marginBottom: '1rem' }}>
            ...
            <div className="action-row">
              <div style={{ flex: '1', minWidth: '150px' }}>
                <label className="field-label">技の種類</label>
                <select
              ...
              <div style={{ flex: '2', minWidth: '200px' }}>
                <label className="field-label">技の名前 (全角10文字以内)</label>
                <input
                  type="text"
                  className="input-field"
                  style={{ width: '100%' }}
        ...
        <div className="grid-2" style={{ marginBottom: '1.5rem', gap: '1rem' }}>
          <div className="inset-panel" style={{ textAlign: 'center' }}>
            <h3 className="sec-title">寄付</h3>
            <p style={{ fontSize: '0.8rem', marginBottom: '1rem', minHeight: '40px' }}>
              「この研究所に寄付してくれて構わんぞ。」<br/>名声をランダムに獲得（0のことも）。
            </p>
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', justifyContent: 'center' }}>
              <select className="input-field" style={{ width: '90px' }} value={baimeiKaisu} onChange={(e) => setBaimeiKaisu(Number(e.target.value))}>
        ...
        <div className="grid-2" style={{ marginBottom: '1.5rem', gap: '1rem' }}>
          <div className="inset-panel">
            <h3 className="sec-title">特性開発室</h3>
            <p style={{ fontSize: '0.8rem', marginBottom: '1rem' }}>お金(1Lv=1000G)と名声(1Lv=10)を消費して特性を開発します。合計Lv上限=熟練度/2です。</p>
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem' }}>
              <select className="input-field" style={{ width: '100%' }} value={traitName} onChange={e => setTraitName(e.target.value)}>
              ...
              <input type="number" className="input-field" style={{ width: '80px' }} value={addLv} min={1} max={10} onChange={e => setAddLv(Number(e.target.value))} />
            </div>
            <button onClick={handleDevelopTrait} className="btn primary block">開発する</button>

            {user.level >= 30 && (
              <div style={{ marginTop: '1.5rem', borderTop: '1px solid var(--border-color)', paddingTop: '1rem' }}>
                <p style={{ fontSize: '0.8rem', color: 'var(--danger)', marginBottom: '0.5rem' }}>特性をすべて削除（無料）</p>
                <button onClick={handleResetTraits} className="btn danger block">すべて削除する</button>
              </div>
            )}
          </div>

          <div className="inset-panel">
            <h3 className="sec-title">能力低減訓練</h3>
            <p style={{ fontSize: '0.8rem', marginBottom: '1rem' }}>伸びすぎたステータスを意図的に下げます。（返金なし）</p>
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem' }}>
              <select className="input-field" style={{ width: '100%' }} value={reduceStatus} onChange={e => setReduceStatus(e.target.value)}>
                <option value="intuition">直感</option>
                <option value="piloting">操縦</option>
                <option value="short_range">近距離</option>
                <option value="mid_range">中距離</option>
            ...
            <button onClick={handleReduceStatus} className="btn danger block">能力を1下げる</button>
          </div>
        </div>
      </div>
    </div>
